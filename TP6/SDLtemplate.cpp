...
int main(int argc, char** argv) {
    ...
    while(!done) {
        ...
        for (int i = 0; i < 32; ++i) {
            glm::mat4 moonMVMatrix = glm::translate(glm::mat4(1.f), glm::vec3(0.f, 0.f, -5.f));
            moonMVMatrix = glm::rotate(moonMVMatrix, windowManager.getTime(), rotationAxes[i]);
            moonMVMatrix = glm::translate(moonMVMatrix, glm::vec3(-2.f, 0.f, 0.f));
            moonMVMatrix = glm::scale(moonMVMatrix, glm::vec3(0.2f, 0.2f, 0.2f));

            // Send matrices to the GPU
            glUniformMatrix4fv(uMVPMatrixLoc, 1, GL_FALSE, glm::value_ptr(ProjMatrix * moonMVMatrix));
            glUniformMatrix4fv(uMVMatrixLoc, 1, GL_FALSE, glm::value_ptr(moonMVMatrix));
            glUniformMatrix4fv(uNormalMatrixLoc, 1, GL_FALSE, glm::value_ptr(glm::transpose(glm::inverse(moonMVMatrix))));
